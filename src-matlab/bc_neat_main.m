...
%list of parameters
  ...
  %parameters of main file
    maxgeneration=500; %maximum number of generations for generational loop    
  ...
  %parameters initial population
    ...
    %parameters for ball catching experiment
    params.environment_width=400;
    params.sensor_length=200;
    params.sensor_width=40;
    params.agent_speed=1;
    params.agent_radius=5;
    params.ball_radius=5;
    params.number_ballpairs=12;
...
if load_flag==0
   ...
   [population,innovation_record]=bc_initial_population(population_size, number_input_nodes, number_hidden_nodes, number_output_nodes);
   ...
   %initial speciation
   number_connections=number_input_nodes*number_hidden_nodes+number_hidden_nodes*number_output_nodes;
   ...
   % species reference matrix (abbreviated, only weights, since there are no topology differences in 
   % initial population)
   ...
   for index_individual=2:size(population,2);
      ...
      while assigned_existing_species_flag==0 & new_species_flag==0 
          %loops through the existing species, terminates when either the individual is assigned to 
          % existing species or there are no more species to test it against, which means it is a new species 
...
while generation<maxgeneration & flag_solution==0
   ...
   [population,catches,misses]=bc_experiment(population,number_input_nodes,number_hidden_nodes,params);
   ...
   for index_individual=1:size(population,2)
      ...
      if cur_max_catch<catches(index_individual)
          cur_max_catch=catches(index_individual);
      end
   end
   ...
   cumul_max_catch=[cumul_max_catch,cur_max_catch];
   c=[];
   for index_species=1:size(species_record,2)
   ...
   %%%%%
   if cur_max_catch==2*params.number_ballpairs
       flag_solution=1;
   end
   %%%%%
   ...
   [population,species_record,innovation_record]=bc_reproduce(population, species_record, innovation_record, initial, selection, crossover, mutation, speciation, generation, population_size);
   toc;
   end
